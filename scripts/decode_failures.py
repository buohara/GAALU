...
# default threshold (raw units). change via 2nd CLI arg
DEFAULT_THRESH = 100
...
def parse_failures(path):
    ...
    # pattern tolerant to whitespace; capture test number and op code too
    pat = re.compile(
...
def print_test(m, thresh=DEFAULT_THRESH):
    ...
    for idx, (name, val) in enumerate(zip(FIELD_NAMES, A_floats)):
        if abs(val) > 1e-6:  # Only show non-zero components
